...
let $elem.id := request:get-parameter('pathID','')
...
let $result := transform:transform($doc,
               doc(concat($xslPath,'queryElement.xsl')), <parameters><param name="elem.id" value="{$elem.id}"/><param name="lang" value="{$language}"/></parameters>)
